...
# Extract Quick Validator Widget (lines 1240-1271)
quick_val_lines = []
in_quick_val = False
for line in lines:
    if '<!-- Quick Validator Widget -->' in line:
        in_quick_val = True
    if in_quick_val:
        quick_val_lines.append(line)
        if '    </div>' in line and '</div>' in quick_val_lines[-2] and '</div>' in quick_val_lines[-3]:
            pass
        # simpler way:
quick_validator_html = "".join(lines[1240:1271])
